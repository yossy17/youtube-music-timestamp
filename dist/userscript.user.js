...
(function() {
  ...
  `;
    ...
    ytmTimestampIcon.src = "https://github.com/yossy17/youtube-music-timestamp/raw/master/images/icons/icon-48.webp";
  ...
  const countLines = (text) => {
    if (!text.trim()) return [];
    const blocks = text.split(/\[\d+:\d{2}\.\d{2,}\]\s*\(ytm-space\)/).filter((block) => block.trim());
